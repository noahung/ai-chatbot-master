...
const TrainingData = () => {
  ...
  const { getClient, addTrainingData, updateTrainingData, deleteTrainingData, updateClientTrainingData } = useChatbot();
  ...
  const processTrainingData = async () => {
    if (!id) {
      toast.error('Client ID is missing. Please refresh the page or select a client.');
      return;
    }
    
    // Check if user is admin before allowing training
    const { data: userData } = await supabase.auth.getUser();
    const user = userData?.user;
    const isAdmin = user?.user_metadata?.role === 'admin';
    
    if (!isAdmin) {
      toast.error('Only administrators can train the AI.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('https://rlwmcbdqfusyhhqgwxrz.supabase.co/functions/v1/process-training', {
        ...
        headers: {
          ...
          'Authorization': `Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJsd21jYmRxZnVzeWhocWd3eHJ6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDcxMzAzMzMsImV4cCI6MjA2MjcwNjMzM30.96HbYy6EfaY2snPjvcO6hT2E-pVCFOvSz5anC3GYVQ8`
        },
      ...
      // Fetch latest training data from Supabase
      ...
      if (error) {
        ...
        if (data) {
          updateClientTrainingData(id, data);
        }
      }
              ...
              {client.trainingData.length > 0 ? (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-end">
                    <AdminButton>
                      <Button onClick={processTrainingData} disabled={isTraining}>
                        {isTraining ? (
                          <>
